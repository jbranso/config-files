...
function display()
   ...
   for group, res in pairs(strings) do
      ...
      result = result ..
	 '<span weight="bold" color="' .. "#009933" .. '">' ..
--	 '<span weight="bold" color="' .. beautiful.fg_widget_value_important .. '">' ..
	 group .. "</span>\n" .. res
   end

   keyboard_shortcuts = "\n\tCrtl+Alt+Del    reboots computer\n" ..
      "\tAlt-Fn-Ctrl-F1, F2, F3, ...    specifies a virtual console to use\n" ..
      "Crlt+c     kill the current task\n" ..
      "Crlt+d     insert an EOF\n" ..
      "Ctrl-z     pause the current job\n" ..
      "M-Alt-r    take control of keyboard back from X\n" ..
      "M-Alt-e    gracefully kill all processes\n" ..
      "M-Alt-i    forcefully kill all processes\n" ..
      "M-Alt-s    flush data to disk\n" ..
      "M-Alt-u    unmount and remount all filesystems as read only\n" ..
      "M-Alt-b   reboot\n"

   nid = naughty.notify({ text = result .. keyboard_shortcuts,
			  replaces_id = nid,
			  hover_timeout = 0.1,
			  timeout = 30 }).id
end
